...
- **设置面板**：可拖动悬浮按钮下方新增「⚙ 设置」小条，点击弹出设置窗口，设置即时保存（SharedPreferences）：
  - **单局时间**：1–4 秒（默认 2 秒），按「目标耗时 ÷ 题数 − H5 固定开销」换算每题回调延迟
    （JS 侧封顶 60ms，更大值无效——避免回调超时导致 H5 不判题卡题）；
...
- **答题卡在同一题（笔迹反复绘制、不跳下一题）**：设置化后的回答回调延迟 `__pkDelay` 无上限，
  单局时间调大时每题回调拖到数百毫秒，H5 识别超时不判题 → 永不进下一题，而画笔循环持续在同一画布重绘。
  修复：回调延迟封顶 **60ms**（`min(__pkDelay, 60) + rand(12)`，默认档 ≈ 原版 10-21ms）、
  笔画步进/笔间节奏**完全回退原版常量**（`2+rand(3)` / `6+rand(8)`）、
  单局时间范围收敛至 **1-4 秒**（读写双侧 clamp）。真机复测：recognize 每 100-200ms 连续推进，
  30 题 4.6 秒完成、服务端判分 `correctCnt=30`。
